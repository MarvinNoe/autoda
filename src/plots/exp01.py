...
base_dir = './outputs/experiments/exp01/val'

data = read_map_values(base_dir)
df = pd.DataFrame(data)

df['Experiment'] = df['Experiment'].replace('baseline_validation', 'Baseline')
df['Experiment'] = df['Experiment'].replace('fastaa_validation', 'FastAA')
df['Experiment'] = df['Experiment'].replace('gc10_augment_validation', 'GC10-DET Augment')
...
custom_palette = ['#179C7D', '#179C7D', '#179C7D', '#179C7D', '#179C7D']
